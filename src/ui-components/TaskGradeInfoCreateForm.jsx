...
import { listSyllabusGradeValues, listTasks } from "../graphql/queries";
import {
  createTaskGradeInfo,
  updateTask,
  updateTaskGradeInfo,
...
export default function TaskGradeInfoCreateForm(props) {
  const {
    clearOnSuccess = true,
    onSuccess,
    onError,
  ...
  const resetStateValues = () => {
    ...
    setTime_Taken(initialValues.time_Taken);
    setTask(initialValues.Task);
    setCurrentTaskValue(undefined);
    setCurrentTaskDisplayValue("");
    setSyllabusgradevaluesID(initialValues.syllabusgradevaluesID);
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          const modelFieldsToSave = {
            ...
            time_Taken: modelFields.time_Taken,
            taskGradeInfoTaskId: modelFields?.Task?.id,
            syllabusgradevaluesID: modelFields.syllabusgradevaluesID,
          };
          const taskGradeInfo = (
            await client.graphql({
              query: createTaskGradeInfo.replaceAll("__typename", ""),
              variables: {
                input: {
                  ...modelFieldsToSave,
                },
            ...
            promises.push(
              client.graphql({
                ...
                variables: {
                  input: {
                    id: Task.id,
                    taskTaskGradeInfoId: taskGradeInfo.id,
                  },
                },
              })
            );
      ...
      {...getOverrideProps(overrides, "TaskGradeInfoCreateForm")}
      {...rest}
    >
      <TextField
        label="Current grade"
