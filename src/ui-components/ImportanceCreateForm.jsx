...
import { createImportance } from "../graphql/mutations";
const client = generateClient();
export default function ImportanceCreateForm(props) {
  const {
    clearOnSuccess = true,
    onSuccess,
    onError,
  ...
  const resetStateValues = () => {
    setGrade_Percentage(initialValues.Grade_Percentage);
    setTask_info(initialValues.Task_info);
    setExpected_Time(initialValues.Expected_Time);
    setCourse(initialValues.Course);
    setAdditional_Info(initialValues.Additional_Info);
    setErrors({});
  };
  const validations = {
    Grade_Percentage: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: createImportance.replaceAll("__typename", ""),
            variables: {
              input: {
                ...modelFields,
              },
      ...
      {...getOverrideProps(overrides, "ImportanceCreateForm")}
      {...rest}
    >
      <TextField
