...
import { getLetterGrade, getSubjects, listSubjects } from "../graphql/queries";
import { updateLetterGrade } from "../graphql/mutations";
...
export default function LetterGradeUpdateForm(props) {
  const {
    id: idProp,
    letterGrade: letterGradeModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = letterGradeRecord
      ? { ...initialValues, ...letterGradeRecord, subjectsID }
      : initialValues;
    setLetterValue(cleanValues.LetterValue);
    setGradeCutoff(cleanValues.GradeCutoff);
    setSubjectsID(cleanValues.subjectsID);
    setCurrentSubjectsIDValue(undefined);
    setCurrentSubjectsIDDisplayValue("");
    setErrors({});
  };
  const [letterGradeRecord, setLetterGradeRecord] =
    React.useState(letterGradeModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getLetterGrade.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getLetterGrade
        : letterGradeModelProp;
      ...
      setLetterGradeRecord(record);
    };
    queryData();
  }, [idProp, letterGradeModelProp]);
  React.useEffect(resetStateValues, [letterGradeRecord, subjectsID]);
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          LetterValue: LetterValue ?? null,
          GradeCutoff: GradeCutoff ?? null,
          subjectsID,
        };
      ...
      {...getOverrideProps(overrides, "LetterGradeUpdateForm")}
      {...rest}
    >
      <TextField
        label="Letter value"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || letterGradeModelProp)}
